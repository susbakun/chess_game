...
pub fn on_square_hover(
    ...
    if let Ok((mut material_handle, _square)) = query.get_mut(_over.entity) {
...
pub fn on_square_hover_end(
    ...
    if let Ok((mut material_handle, square)) = query.get_mut(_out.entity) {
...
pub fn on_sqaure_click(
    ...
    if let Some(prev_square_entity) = selected_square.entity {
        if prev_square_entity != _click.entity {
            if let Ok((mut prev_material, prev_square)) = squares_query.get_mut(prev_square_entity)
            {
    ...
    if let Ok((mut material_handle, _square)) = squares_query.get_mut(_click.entity) {

...
#[cfg(not(target_arch = "wasm32"))]
pub fn init_difficulty_menu(mut commands: Commands, asset_server: Res<AssetServer>) {
...
pub fn show_difficulty_menu(
    ...
    if let Some(game_type) = &game_state.game_type {
        if *game_type == GameType::PlayWithAi && game_state.difficulty.is_none() {
...
pub fn hide_difficulty_menu(
    ...
    if let Some(game_type) = &game_state.game_type {
        if *game_type == GameType::PlayWithAi && game_state.difficulty.is_some() {
            for (mut visibility, _) in background_query.iter_mut() {

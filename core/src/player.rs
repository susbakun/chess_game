...
impl Player {
    pub fn is_check(&self, pieces: &Vec<Piece>) -> bool {
        let enemy_pieces = pieces.iter().filter(|piece| piece.color != self.0);

        let king = pieces
            .iter()
            .find(|piece| piece.piece_type == PieceType::King && piece.color == self.0)
        ...
        for piece in enemy_pieces {
            match piece.piece_type {
                ...
                PieceType::Bishop => {
                    if ((king.x - piece.x).abs() == (king.y - piece.y).abs())
                        && is_path_empty((piece.x, piece.y), (king.x, king.y), &pieces)
                    {
                        return true;
                    }
                ...
                PieceType::King => {
                    if ((king.x - piece.x).abs() == 1 && (king.y == piece.y))
                        || ((king.y - piece.y).abs() == 1 && (king.x == piece.x))
                        || ((king.x - piece.x).abs() == 1 && (king.y - piece.y).abs() == 1)
                    {
                        return true;
                    }
                }
                PieceType::Knight => {
                    if ((king.x - piece.x).abs() == 2 && (king.y - piece.y).abs() == 1)
                        || ((king.x - piece.x).abs() == 1 && (king.y - piece.y).abs() == 2)
                    {
                        return true;
                    }
                }
